...
The autonomous flow depends on the **`tce-unit-builder`** skill, installed at
`~/.claude/skills/tce-unit-builder/`. The skill encodes the per-stage workflow so Claude can
resume work without rediscovering the runbook each time. It is **not** part of this repo —
maintain it under your home directory and version it separately.

The skill is documented at `~/.claude/skills/tce-unit-builder/SKILL.md`. It triggers on
phrases like "continue the unit", "next stage", "resume the worksheet generator", and on
scheduled cron firings.

A long-form project memory file also lives outside the repo at
`~/.claude/projects/-Users-<you>-...-worksheet_generator/memory/` and contains the
architectural decisions, layout invariants, and grade-density rules referenced by `CLAUDE.md`.
...
- `~/.claude/skills/tce-unit-builder/` — the skill that drives autonomous runs.
- `~/.claude/projects/.../memory/` — long-form project memory.
